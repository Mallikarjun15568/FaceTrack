...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("userFilter").addEventListener("change", loadAttendance);
...
function renderAttendance(records) {
    ...
    records.forEach(r => {
        ...
        if (r.status === "present") {
            statusBadge = `<span class="px-2 py-1 text-xs rounded bg-green-100 text-green-700">Present</span>`;
        } else if (r.status === "late") {
            statusBadge = `<span class="px-2 py-1 text-xs rounded bg-yellow-100 text-yellow-700">Late</span>`;
        } else if (r.status === "early_leave") {
            statusBadge = `<span class="px-2 py-1 text-xs rounded bg-red-100 text-red-700">Early Leave</span>`;
        } else if (r.status === "check-in") {
            statusBadge = `<span class="px-2 py-1 text-xs rounded bg-sky-100 text-sky-700">Check-in</span>`;
        } else if (r.status === "check-out") {
            statusBadge = `<span class="px-2 py-1 text-xs rounded bg-green-100 text-green-700">Check-out</span>`;
        } else if (r.status === "already") {
            statusBadge = `<span class="px-2 py-1 text-xs rounded bg-gray-100 text-gray-700">Already marked</span>`;
        }
        ...
        tr.innerHTML = `
            ...
            <td class="p-3">
                <img src="${snapshot}"
                     class="w-12 h-12 rounded-lg object-cover border shadow cursor-pointer"
